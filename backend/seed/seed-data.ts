...
export function generateSeedData(): SeedData {
  ...
  const cases: RecoveryCase[] = [
    {
      ...
      diagnosis: { rootCauseCategory: 'LIMIT_EXCEEDED', rootCauseDetail: 'Customer exceeded single-transaction UPI ticket limit on HDFC Bank handle.', confidenceScore: 0.94, isTransient: false, bankCode: 'HDFC', bankSwitchHealthIndex: 94.8, recommendedRailSwitch: 'CARD', diagnosedAt: new Date(Date.now() - 3590000).toISOString() },
      strategy: { recommendedAction: 'ACP_A2A_OFFER', targetChannel: 'WHATSAPP', offeredDiscountPct: 5.0, calculatedIncentiveINR: 249.95, delayMinutes: 0, reasoning: 'High-value Platinum user facing single-transaction UPI limit. Proposed instant 5% cashback on tokenized Visa card checkout.', expectedRecoveryProbability: 0.91, scheduledExecutionAt: new Date(Date.now() - 3580000).toISOString() },
      acpSession: {
      ...
      compliance: { approved: true, rulesPassed: ['TRAI_QUIET_HOURS_OK', 'MAX_DISCOUNT_WITHIN_CAP', 'COMMUNICATION_FATIGUE_OK'], violations: [], requiresHumanApproval: false, evaluatedAt: new Date(Date.now() - 3530000).toISOString() },
    ...
    },
    {
      ...
      diagnosis: { rootCauseCategory: 'ISSUER_DOWNTIME', rootCauseDetail: 'SBI Corporate Banking switch is currently degraded (Success rate 68.2%).', confidenceScore: 0.96, isTransient: true, bankCode: 'SBI', bankSwitchHealthIndex: 68.2, recommendedRailSwitch: 'CARD', diagnosedAt: new Date(Date.now() - 1790000).toISOString() },
      strategy: { recommendedAction: 'ACP_A2A_OFFER', targetChannel: 'WHATSAPP', offeredDiscountPct: 8.0, calculatedIncentiveINR: 3880.00, delayMinutes: 15, reasoning: 'High-value enterprise order (₹48,500). System flagged for Human Approval due to transaction value exceeding ₹25,000 threshold.', expectedRecoveryProbability: 0.85, scheduledExecutionAt: new Date(Date.now() - 1780000).toISOString() },
      compliance: { approved: false, rulesPassed: ['TRAI_QUIET_HOURS_OK'], violations: ['TRANSACTION_EXCEEDS_AUTO_APPROVAL_THRESHOLD (₹48,500 > ₹25,000)'], requiresHumanApproval: true, evaluatedAt: new Date(Date.now() - 1770000).toISOString() },
    ...
    },
    {
      ...
      diagnosis: { rootCauseCategory: 'MANDATE_EXPIRED', rootCauseDetail: 'Customer e-Mandate expired at issuing bank. Requires tokenized card mandate renewal.', confidenceScore: 0.98, isTransient: false, bankCode: 'ICICI', bankSwitchHealthIndex: 96.1, recommendedRailSwitch: 'CARD', diagnosedAt: new Date(Date.now() - 890000).toISOString() },
      strategy: { recommendedAction: 'ACP_A2A_OFFER', targetChannel: 'WHATSAPP', offeredDiscountPct: 0.0, calculatedIncentiveINR: 0.0, delayMinutes: 0, reasoning: 'Recurring subscription mandate renewal required. ACP agent dispatches instant 1-click token authorization link.', expectedRecoveryProbability: 0.88, scheduledExecutionAt: new Date(Date.now() - 880000).toISOString() },
      acpSession: {
      ...
      compliance: { approved: true, rulesPassed: ['TRAI_QUIET_HOURS_OK', 'FREQUENCY_LIMIT_OK'], violations: [], requiresHumanApproval: false, evaluatedAt: new Date(Date.now() - 860000).toISOString() },
    ...
    // 2b. Checkout Abandonment Cases
    {
      ...
      diagnosis: { rootCauseCategory: 'CHECKOUT_STALL', rootCauseDetail: 'Customer stalled at UPI payment authorization for 8+ minutes on mobile device.', confidenceScore: 0.91, isTransient: false, bankCode: 'HDFC', bankSwitchHealthIndex: 94.8, recommendedRailSwitch: 'CARD', diagnosedAt: new Date(Date.now() - 1790000).toISOString() },
      strategy: { recommendedAction: 'PAYMENT_LINK_DISPATCH', targetChannel: 'WHATSAPP', offeredDiscountPct: 3.0, calculatedIncentiveINR: 224.97, delayMinutes: 0, reasoning: 'High cart value (₹7,499) with 3-item basket and Gold CLV tier. Recovery probability 78%.', expectedRecoveryProbability: 0.82, scheduledExecutionAt: new Date(Date.now() - 1780000).toISOString() },
      compliance: { approved: true, rulesPassed: ['TRAI_QUIET_HOURS_OK', 'MAX_DISCOUNT_WITHIN_CAP', 'CHECKOUT_RECOVERY_AUTHORIZED'], violations: [], requiresHumanApproval: false, evaluatedAt: new Date(Date.now() - 1770000).toISOString() },
    ...
    // 2c. B2B Receivables Invoice Cases
    {
      ...
      diagnosis: { rootCauseCategory: 'INVOICE_APPROVAL_DELAY', rootCauseDetail: 'Internal procurement approval delayed by 2 weeks at TechSolutions.', confidenceScore: 0.94, isTransient: false, bankCode: 'HDFC', bankSwitchHealthIndex: 94.8, recommendedRailSwitch: 'NETBANKING', diagnosedAt: new Date(Date.now() - 3600000).toISOString() },
      ...
      compliance: { approved: true, rulesPassed: ['TRAI_QUIET_HOURS_OK', 'B2B_INVOICING_COMPLIANT', 'VALUE_WITHIN_AUTO_THRESHOLD'], violations: [], requiresHumanApproval: false, evaluatedAt: new Date(Date.now() - 3400000).toISOString() },
    ...
    },
    {
      ...
      diagnosis: { rootCauseCategory: 'INVOICE_CASHFLOW_ISSUE', rootCauseDetail: 'Precision Manufacturing reports Q2 cash flow constraints.', confidenceScore: 0.91, isTransient: false, bankCode: 'ICICI', bankSwitchHealthIndex: 96.1, recommendedRailSwitch: 'NETBANKING', diagnosedAt: new Date(Date.now() - 3600000).toISOString() },
      strategy: { recommendedAction: 'PAYMENT_LINK_DISPATCH', targetChannel: 'EMAIL', offeredDiscountPct: 2.0, calculatedIncentiveINR: 8400, delayMinutes: 0, reasoning: 'High-value ₹4.2L invoice at 92+ DPD with cash flow root cause. Offer 2% early payment discount.', expectedRecoveryProbability: 0.72, scheduledExecutionAt: new Date(Date.now() - 3500000).toISOString() },
      compliance: { approved: true, rulesPassed: ['TRAI_QUIET_HOURS_OK', 'B2B_INVOICING_COMPLIANT'], violations: [], requiresHumanApproval: false, evaluatedAt: new Date(Date.now() - 3400000).toISOString() },
    ...
    // Voice Recovery Agent Seed Cases
    {
      ...
      diagnosis: { rootCauseCategory: 'INSUFFICIENT_FUNDS', rootCauseDetail: 'Customer UPI transaction failed due to insufficient funds.', confidenceScore: 0.92, isTransient: true, bankCode: 'SBI', bankSwitchHealthIndex: 97.2, recommendedRailSwitch: 'UPI', diagnosedAt: new Date(Date.now() - 41500000).toISOString() },
      strategy: { recommendedAction: 'VOICE_CALL', targetChannel: 'VOICE', offeredDiscountPct: 0, calculatedIncentiveINR: 0, delayMinutes: 0, reasoning: 'Gold CLV customer with transient insufficient funds.', expectedRecoveryProbability: 0.85, scheduledExecutionAt: new Date(Date.now() - 41000000).toISOString() },
      compliance: { approved: true, rulesPassed: ['TRAI_QUIET_HOURS_OK', 'VOICE_CALL_CONSENT_OBTAINED', 'DO_NOT_DISTURB_CLEAR'], violations: [], requiresHumanApproval: false, evaluatedAt: new Date(Date.now() - 40500000).toISOString() },
    ...
    },
    {
      ...
      diagnosis: { rootCauseCategory: 'STICKY_CHECKOUT', rootCauseDetail: 'High-value cart abandoned at payment page on desktop.', confidenceScore: 0.88, isTransient: true, bankCode: 'HDFC', bankSwitchHealthIndex: 94.8, recommendedRailSwitch: 'UPI', diagnosedAt: new Date(Date.now() - 6500000).toISOString() },
      strategy: { recommendedAction: 'VOICE_CALL', targetChannel: 'VOICE', offeredDiscountPct: 5, calculatedIncentiveINR: 749.95, delayMinutes: 0, reasoning: 'Platinum CLV customer with high cart value.', expectedRecoveryProbability: 0.88, scheduledExecutionAt: new Date(Date.now() - 6200000).toISOString() },
      compliance: { approved: true, rulesPassed: ['TRAI_QUIET_HOURS_OK', 'VOICE_CALL_CONSENT_OBTAINED', 'DISCOUNT_WITHIN_THRESHOLD'], violations: [], requiresHumanApproval: false, evaluatedAt: new Date(Date.now() - 6100000).toISOString() },
  ...
  const auditEntries: Omit<AuditLogEntry, 'id' | 'signatureHash' | 'timestamp'>[] = [
    { caseId: 'REC-2026-881', agentName: 'Detection Agent', action: 'INGEST_FAILURE_EVENT', rationale: 'Ingested Razorpay webhook payment.failed for ₹4,999. Customer CLV tier evaluated as PLATINUM.', model: 'gemini-3.7-flash', latencyMs: 140, tokensUsed: 210 },
    { caseId: 'REC-2026-881', agentName: 'Diagnosis Agent', action: 'CORRELATE_BANK_HEALTH', rationale: 'Correlated with HDFC switch health (94.8% healthy). Pinpointed user-level daily ticket limit exhaustion.', model: 'gemini-3.7-flash', latencyMs: 280, tokensUsed: 390 },
    { caseId: 'REC-2026-881', agentName: 'Strategy Optimizer Agent', action: 'FORMULATE_ACP_OFFER', rationale: 'Calculated Expected Value of 5% instant discount at ₹4,749 > ₹0 default churn loss.', model: 'gemini-3.7-flash', latencyMs: 410, tokensUsed: 580 },
